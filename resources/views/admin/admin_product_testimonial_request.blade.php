...
{
  if (confirm("Are you sure want to reject " + name + ":\n" + testi + " ?") == true) 
  {
    $.ajax({
      type: "POST",
      url: "{{ URL::to('/admin/process/testimoni') }}",
...
function approve(id, name, testi) 
{
  if (confirm("Are you sure want to approve " + name + ":\n" + testi + " ?") == true) 
  {
    $.ajax({
      type: "POST",
      url: "{{ URL::to('/admin/process/testimoni') }}",
